...
const ProductsContentV2 = () => {
  ...
  const totalPages = productsData?.totalPages || Math.ceil(totalProducts / limitParam) || 1;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (tabParam === "in-stock") {
      list = list.filter((p) => {
        const stock = typeof p.stock === "number" ? p.stock : parseInt(p.stock, 10);
  ...
  return (
    <div className="pb-24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <Breadcrumbs title="Products (V2 Redesign)" />

        <div className="flex items-center gap-2">
          <span className="badge badge-primary badge-soft text-xs font-semibold px-2.5 py-1">
            V2 Redesign
          </span>
          <Link
            href="/dashboard/products"
            className="btn btn-xs btn-ghost border border-base-content/15 hover:border-base-content/30 gap-1 text-xs text-base-content/70"
            title="Open Original Products Page"
          >
            Switch to Original V1
